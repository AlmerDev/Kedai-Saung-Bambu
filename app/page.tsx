...
export default function HomePage() {
  ...
  function openSnapPayment(order: Order, token: string) {
    setPaymentDialog({
      ...
      description:
        "Pilih QRIS, e-wallet, atau metode lain di popup pembayaran. Kalau popup tertutup, kamu bisa klik Bayar Lagi.",
      order,
      canRetry: true,
    });
    notify(
  ...
  async function retryPayment(order: Order) {
    try {
      if (!midtransClientKey)
        throw new Error("Midtrans belum aktif. Isi ENV Midtrans dulu.");
      if (!window.snap)
        throw new Error(
          "Snap Midtrans belum siap. Coba klik lagi beberapa detik.",
        );
              ...
              <div className="overflow-hidden rounded-[2.25rem] border border-white/70 bg-white shadow-2xl shadow-orange-950/10">
                ...
                <div className="space-y-4 p-5">
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      {submitting ? "Memproses..." : "Bayar Online Midtrans"}
                    </button>
                    <button
                      disabled={submitting || cartLines.length === 0}
                      onClick={handleCash}
                      className="rounded-2xl border-2 border-saung-red bg-white px-5 py-3 font-black text-saung-red transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      Bayar di Kasir
                    </button>
                    {settings.whatsapp ? (
        ...
        <div className="space-y-4 p-5">
          ...
          {dialog.canRetry && onRetry ? (
            <button
              onClick={onRetry}
              disabled={retrying}
              className="w-full rounded-2xl bg-saung-red px-5 py-3 font-black text-white shadow-lg transition hover:bg-saung-dark disabled:cursor-not-allowed disabled:opacity-60"
            >
