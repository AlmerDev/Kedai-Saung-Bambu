...
export default function HomePage() {
  ...
  function goToCart() {
    if (cart.length === 0) {
      notify(
        ...
        "Pilih menu dulu, nanti checkout muncul di halaman terpisah.",
      );
      return;
    }
    setView("cart");
  ...
  const products = useMemo(
    () =>
      data.products.filter(
        (item) => category === "semua" || item.category_id === category,
      ),
  ...
  async function readApiJson(response: Response) {
    ...
    try {
      ...
      throw new Error(
        `Response server kosong/tidak valid JSON. Status ${response.status}. Cek terminal lokal atau Vercel Function Logs.`,
      );
    }
  }
  ...
  function addToCart(productId: string) {
    ...
    notify(
      ...
      "Buka checkout dari tombol keranjang di atas/bawah.",
    );
  }
  ...
  async function createOrder(paymentMethod: "cash" | "midtrans") {
    if (cart.length === 0)
      throw new Error("Keranjang masih kosong. Pilih menu dulu ya.");
  ...
  async function handleCash() {
    try {
      ...
      setPaymentDialog({
        ...
        description: "Sebentar ya, pesanan sedang dikirim ke admin dapur.",
      });
      ...
      setPaymentDialog({
        ...
        description:
          "Sebutkan kode pesanan ini saat membayar di kasir. Admin sudah bisa melihat pesanan kamu.",
        order,
      });
      notify(
        "success",
      ...
      setPaymentDialog({
        type: "error",
        title: "Pesanan belum bisa dikirim",
        description: message,
      });
  ...
  function canRetryPayment(order: Order | null | undefined) {
    ...
    return (
      ...
      !["lunas", "refund"].includes(order.payment_status) &&
  ...
  function paymentDescription(order: Order) {
    if (order.payment_status === "lunas")
      return "Status order otomatis berubah menjadi lunas dan pesanan masuk ke dapur untuk diproses.";
    if (order.payment_status === "gagal")
      return "Transaksi belum berhasil. Kamu bisa membuka ulang pembayaran atau bayar langsung di kasir.";
    if (order.payment_status === "expire")
      return "Waktu pembayaran sudah habis. Kamu bisa membuka ulang pembayaran dari tombol Bayar Lagi.";
    if (order.payment_status === "refund")
      return "Transaksi tercatat refund. Hubungi admin kedai untuk pengecekan.";
  ...
  async function fetchOrder(orderId: string) {
    ...
    const payload = (await readApiJson(response)) as any;
    if (!response.ok)
      throw new Error(payload.error || "Gagal mengambil detail order.");
    return payload.order as Order;
  }

  async function syncPayment(orderId: string) {
    const response = await fetch(`/api/orders/${orderId}/sync-payment`, {
      method: "POST",
    });
    const payload = (await readApiJson(response)) as any;
    if (!response.ok)
      throw new Error(payload.error || "Gagal mengecek status pembayaran.");
    return payload.order as Order;
  }
  ...
  async function loadReturnedOrder(orderId: string) {
    try {
      setPaymentDialog({
        ...
        description:
          "Sebentar ya, status pembayaran sedang dicocokkan dengan Midtrans.",
      });
  ...
  async function prepareMidtrans(orderId: string) {
    ...
    return {
      token,
      redirectUrl,
      order: (payload.order || null) as Order | null,
    };
  }
  ...
  function openSnapPayment(
    ...
    setPaymentDialog({
      ...
      description:
        "Pilih QRIS, e-wallet, atau metode lain. Kalau popup tidak muncul, klik tombol Buka Halaman Pembayaran.",
      order,
      canRetry: true,
      redirectUrl,
    });
    notify(
      ...
      "Kalau popup Midtrans tidak muncul, gunakan tombol Buka Halaman Pembayaran.",
    );
    ...
    if (!window.snap || !token) {
      if (redirectUrl) {
        notify(
          ...
          "Popup belum siap, jadi pembayaran dibuka lewat halaman Midtrans.",
        );
        openRedirectPayment(redirectUrl);
        return;
      }
      throw new Error(
        "Snap Midtrans belum siap. Coba klik Bayar Lagi beberapa detik lagi.",
      );
    }
    ...
    try {
      window.snap.pay(token, {
        onSuccess: async () => {
          ...
          setPaymentDialog({
            type:
              syncedOrder.payment_status === "lunas" ? "success" : "pending",
          ...
          notify(
            syncedOrder.payment_status === "lunas" ? "success" : "info",
            paymentTitle(syncedOrder),
            paymentDescription(syncedOrder),
          );
        },
        onPending: async () => {
          const syncedOrder = await getLatestMidtransOrder(order.id);
          ...
          notify(
            "info",
            "Menunggu pembayaran",
            "Pesanan tersimpan. Kamu bisa melanjutkan pembayaran lagi kalau popup tertutup.",
          );
        },
        onError: async () => {
          const syncedOrder = await getLatestMidtransOrder(order.id);
          ...
          notify(
            ...
            "Silakan klik Bayar Lagi atau bayar di kasir.",
          );
        },
        onClose: async () => {
          const syncedOrder = await getLatestMidtransOrder(order.id);
          ...
          setPaymentDialog({
            ...
            description:
              "Popup Midtrans tertutup sebelum pembayaran selesai. Pesanan tetap tersimpan, dan kamu bisa mengulang pembayaran dari tombol Bayar Lagi.",
            order: syncedOrder,
            canRetry: canRetryPayment(syncedOrder),
            redirectUrl: syncedOrder.midtrans_redirect_url,
          });
          notify(
            "info",
            "Pembayaran ditutup",
            "Klik Bayar Lagi untuk membuka ulang pembayaran.",
          );
        },
      });
    } catch (err) {
      if (redirectUrl) {
        notify(
          "info",
          "Membuka halaman pembayaran",
          "Popup gagal dibuka, jadi pembayaran dialihkan ke halaman Midtrans.",
        );
        openRedirectPayment(redirectUrl);
        return;
      }
  ...
  async function retryPayment(order: Order) {
    try {
      ...
      setPaymentDialog({
        ...
        description:
          "Token pembayaran sedang disiapkan lagi. Kamu bisa melanjutkan pembayaran tanpa membuat pesanan baru.",
        order,
        canRetry: true,
      });
      ...
      setPaymentDialog({
        type: "error",
        title: "Pembayaran belum bisa dibuka",
        description: message,
        order,
        canRetry: canRetryPayment(order),
      });
  ...
  async function handleMidtrans() {
    ...
    try {
      ...
      setPaymentDialog({
        ...
        description:
          "Pesanan sedang dibuat dan token pembayaran Midtrans sedang disiapkan.",
      });
      ...
      setPaymentDialog({
        ...
        description: message,
        order: createdOrder,
        canRetry: canRetryPayment(createdOrder),
      });
  ...
  const whatsappText = encodeURIComponent(
    `Halo ${settings.store_name}, saya mau tanya menu.`,
  );
  const whatsappUrl = settings.whatsapp
    ? `https://wa.me/${settings.whatsapp.replace(/\D/g, "")}?text=${whatsappText}`
    : "#";
    ...
    <main className="relative min-h-screen overflow-x-hidden text-saung-dark">
      ...
      {paymentDialog ? (
        <PaymentModal
          ...
          onRetry={
            paymentDialog.order
              ? () => retryPayment(paymentDialog.order!)
              : undefined
          }
          retrying={submitting}
          whatsappUrl={settings.whatsapp ? whatsappUrl : undefined}
        />
      ) : null}
      {submitting ? (
        <CustomerProcessBanner
          title={paymentDialog?.title || "Memproses pesanan"}
          description={
            paymentDialog?.description ||
            "Mohon tunggu, sistem sedang menyiapkan data pesanan."
          }
        />
      ) : null}
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] font-bold text-orange-950/55">
                  QR Menu • Meja {tableNumber}
                </p>
              </div>
            </button>
            <div className="flex shrink-0 items-center gap-2">
              <button
                onClick={goToCart}
                className="relative rounded-2xl bg-saung-dark px-4 py-2 text-sm font-black text-white shadow-lg transition hover:bg-saung-red"
              >
                Keranjang
                {totalItems > 0 ? (
                  <span className="ml-2 rounded-full bg-saung-yellow px-2 py-0.5 text-xs text-saung-dark">
                    {totalItems}
                  </span>
                ) : null}
              </button>
          ...
          <>
            <section className="relative mb-7 overflow-hidden rounded-[2.5rem] border border-white/60 bg-[#5f050b] p-5 text-white shadow-2xl shadow-red-950/20 sm:p-8 lg:p-10">
              {storeHero ? (
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-[#5f050b] via-saung-red to-saung-orange" />
              )}
              ...
              <div className="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
                <div>
                  <div className="mb-6 flex items-center gap-4 rounded-[1.6rem] border border-white/15 bg-white/12 p-3 backdrop-blur-xl sm:w-fit sm:pr-6">
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-saung-yellow">
                        QR Menu Digital
                      </p>
                  ...
                  <div className="mb-5 flex flex-wrap gap-2">
                    <span className="rounded-full border border-white/20 bg-white/15 px-4 py-2 text-xs font-black backdrop-blur">
                      🍗 Ayam Bakar & Goreng
                    </span>
                    <span className="rounded-full border border-white/20 bg-white/15 px-4 py-2 text-xs font-black backdrop-blur">
                      🥤 Minuman Segar
                  ...
                  <h1 className="max-w-3xl text-4xl font-black leading-[0.98] tracking-tight sm:text-6xl lg:text-7xl">
                    Makan santai rasa saung, pesan cukup dari meja.
                  </h1>
                  ...
                  <div className="mt-7 grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="rounded-[1.4rem] border border-white/20 bg-white/15 p-4 backdrop-blur">
                      ...
                      <p className="mt-1 font-bold">
                        🕒 {settings.opening_hours || "Jam buka belum diisi"}
                      </p>
                    </div>
                  </div>
                  <div className="mt-7 flex flex-wrap gap-3">
                    <a
                      href="#menu"
                      className="rounded-2xl bg-white px-5 py-3 text-sm font-black text-saung-red shadow-xl transition hover:-translate-y-0.5"
                    >
                      Lihat Menu
                    </a>
                    <button
                      onClick={goToCart}
                      className="rounded-2xl border border-white/30 bg-white/15 px-5 py-3 text-sm font-black text-white backdrop-blur transition hover:bg-white/25"
                    >
                      Checkout Pesanan
                    </button>
                  </div>
                  ...
                  <div className="rounded-[2.25rem] border border-white/20 bg-white/15 p-3 backdrop-blur-xl">
                    <div className="overflow-hidden rounded-[1.8rem] bg-saung-cream text-saung-dark shadow-2xl">
                      <div className="relative grid min-h-80 place-items-center overflow-hidden bg-gradient-to-br from-yellow-200 via-orange-200 to-red-200 p-8">
                        ...
                        <div
                          className={`absolute inset-0 ${storeHero ? "bg-gradient-to-t from-white via-white/70 to-white/15" : "food-dots opacity-55"}`}
                        />
                        <div className="relative text-center">
                          ...
                          <p className="text-xs font-black uppercase tracking-[0.3em] text-saung-red">
                            Profil Toko
                          </p>
                          <h2 className="mt-2 text-3xl font-black">
                            {settings.store_name}
                          </h2>
                          <p className="mx-auto mt-2 max-w-xs text-sm font-semibold text-orange-950/65">
                            {settings.tagline ||
                              "Ayam bakar, ayam goreng, minuman segar, dan cemilan ala saung."}
                          </p>
                        </div>
                      </div>
                      <div className="grid grid-cols-3 divide-x divide-orange-100 bg-white p-4 text-center">
                        <div>
                          ...
                          <p className="text-[11px] font-bold text-orange-950/55">
                            Menu
                          </p>
                        </div>
                        <div>
                ...
                <div className="grid gap-4 p-5 sm:grid-cols-[1fr_auto] sm:items-center">
                  <div>
                    <p className="text-xs font-black uppercase tracking-wider text-green-700">
                      Pesanan terakhir berhasil dibuat
                    </p>
                    <h2 className="mt-1 text-2xl font-black text-green-950">
                      #{lastOrder.order_code}
                    </h2>
                    <p className="mt-1 text-sm font-semibold text-green-900/70">
                      Meja {lastOrder.table_number} • Total{" "}
                      {rupiah(lastOrder.total)} •{" "}
                      {lastOrder.payment_method === "midtrans"
                        ? "Online/Midtrans"
                        : "Bayar di kasir"}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 sm:justify-end">
                    {canRetryPayment(lastOrder) ? (
                      <button
                        onClick={() => retryPayment(lastOrder)}
                        className="rounded-2xl bg-saung-red px-5 py-3 text-sm font-black text-white shadow-lg"
                      >
                        Bayar Lagi
                      </button>
                    ) : null}
                        ...
                        setPaymentDialog({
                          type:
                            lastOrder.payment_method === "cash"
                              ? "cash"
                              : lastOrder.payment_status === "lunas"
                                ? "success"
                                : "pending",
                          title: paymentTitle(lastOrder),
                          description: paymentDescription(lastOrder),
                          order: lastOrder,
                          canRetry: canRetryPayment(lastOrder),
                        })
                      }
            ...
            <section id="menu" className="space-y-5">
              <div className="flex flex-col gap-4 rounded-[2rem] border border-white/70 bg-white/75 p-4 shadow-xl shadow-orange-950/5 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-3xl font-black tracking-tight text-saung-dark">
              ...
              {featuredProducts.length ? (
                <section className="grid gap-4 lg:grid-cols-3">
                  {featuredProducts.map((product, index) => (
                    <article
                      key={product.id}
                      className="relative overflow-hidden rounded-[2rem] bg-saung-dark p-5 text-white shadow-xl shadow-orange-950/10"
                    >
                      <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-saung-orange/45 blur-2xl" />
                      <p className="relative text-xs font-black uppercase tracking-wider text-saung-yellow">
                        Rekomendasi #{index + 1}
                      </p>
                      <h3 className="relative mt-2 line-clamp-2 text-xl font-black">
                        {product.name}
                      </h3>
                      <div className="relative mt-4 flex items-end justify-between gap-3">
                        <p className="text-2xl font-black text-saung-yellow">
                          {rupiah(product.price)}
                        </p>
                        <button
                          onClick={() => addToCart(product.id)}
                          className="rounded-2xl bg-white px-4 py-2 text-sm font-black text-saung-red"
                        >
                          Tambah
                        </button>
                      </div>
              ...
              {loading ? (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                      key={i}
                      className="h-72 animate-pulse rounded-[2rem] bg-white/75 shadow"
                    />
                  ))}
                </div>
              ) : products.length === 0 ? (
                <div className="rounded-[2rem] border border-dashed border-orange-300 bg-white/80 p-10 text-center shadow-xl">
                  ...
                  <p className="mt-2 text-sm font-semibold text-orange-950/60">
                    Cek kategori lain atau tambah menu dari admin panel.
                  </p>
                </div>
              ) : (
          ...
          <section className="grid gap-6 lg:grid-cols-[1fr_420px]">
            <div className="space-y-5">
              <div className="rounded-[2.25rem] border border-white/70 bg-white/80 p-5 shadow-xl shadow-orange-950/10 backdrop-blur-xl sm:p-7">
                <button
                  onClick={backToMenu}
                  className="mb-4 rounded-2xl bg-orange-50 px-4 py-2 text-sm font-black text-saung-red"
                >
                  ← Kembali ke menu
                </button>
                <p className="text-xs font-black uppercase tracking-[0.25em] text-saung-orange">
                ...
                <p className="mt-3 max-w-2xl text-sm font-semibold leading-relaxed text-orange-950/60">
                  Atur jumlah, catatan item, data pemesan, lalu pilih
                  pembayaran. Halaman ini dibuat terpisah supaya menu tidak
                  sesak dan checkout lebih rapi.
                </p>
              </div>
              ...
              <div className="rounded-[2.25rem] border border-white/70 bg-white/80 p-4 shadow-xl shadow-orange-950/10 backdrop-blur-xl sm:p-5">
                {cartLines.length === 0 ? (
                  <div className="grid min-h-72 place-items-center rounded-[1.8rem] border border-dashed border-orange-300 bg-orange-50 p-8 text-center">
                    <div>
                      <div className="mx-auto mb-4 grid h-20 w-20 place-items-center rounded-[2rem] bg-white text-5xl shadow">
                        🧺
                      </div>
                      <h2 className="text-2xl font-black">Keranjang kosong</h2>
                      <p className="mt-2 text-sm font-semibold text-orange-950/60">
                        Balik ke menu dan pilih makanan dulu.
                      </p>
                      <button onClick={backToMenu} className="btn-primary mt-5">
                        Pilih Menu
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {cartLines.map((item) => (
                      <article
                        key={item.product_id}
                        className="grid gap-4 rounded-[1.8rem] border border-orange-100 bg-white p-4 shadow-sm sm:grid-cols-[88px_1fr_auto] sm:items-start"
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-sm font-black text-saung-red">
                          ...
                          <input
                            ...
                            className="mt-3 w-full rounded-2xl border border-orange-100 bg-orange-50 px-4 py-3 text-sm font-semibold outline-none focus:border-saung-orange focus:bg-white"
                            placeholder="Catatan item: pedas, tanpa sambal, dll"
                          />
                        </div>
                        <div className="flex items-center justify-between gap-2 rounded-2xl bg-orange-50 p-2 sm:flex-col sm:bg-transparent sm:p-0">
                          <button
                            ...
                            className="grid h-10 w-10 place-items-center rounded-full bg-white font-black text-saung-red shadow"
                          >
                            +
                          </button>
              ...
              <div className="overflow-hidden rounded-[2.25rem] border border-white/70 bg-white shadow-2xl shadow-orange-950/10">
                <div className="bg-gradient-to-br from-saung-red via-saung-orange to-saung-yellow p-5 text-white">
                  ...
                  <p className="mt-2 text-sm font-semibold text-white/80">
                    {totalItems} item • {cartLines.length} menu berbeda
                  </p>
                </div>
                <div className="space-y-4 p-5">
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                    <div>
                      <label className="mb-1 block text-[11px] font-black uppercase tracking-wider text-orange-950/55">
                        Nomor Meja
                      </label>
                      <select
                        value={tableNumber}
                  ...
                  <div className="space-y-2 rounded-[1.6rem] bg-saung-dark p-5 text-white">
                    ...
                    <div className="flex justify-between text-sm">
                      <span>
                        Service fee {settings.service_fee_percent || 0}%
                      </span>
                      <b>{rupiah(serviceFee)}</b>
                    </div>
                    <div className="flex justify-between border-t border-white/15 pt-4 text-xl">
                      <span className="font-black">Total</span>
                      <b className="text-saung-yellow">{rupiah(total)}</b>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      {submitting ? <MiniSpinner light /> : <span>💳</span>}
                      <span>
                        {submitting
                    ...
                    <button
                      disabled={submitting || cartLines.length === 0}
                      onClick={handleCash}
                      className="flex items-center justify-center gap-2 rounded-2xl border-2 border-saung-red bg-white px-5 py-3 font-black text-saung-red transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {submitting ? <MiniSpinner /> : <span>🧾</span>}
                      <span>
                        {submitting ? "Mengirim pesanan..." : "Bayar di Kasir"}
                      </span>
                    </button>
                    {settings.whatsapp ? (
                      <a
                        href={whatsappUrl}
                        target="_blank"
                        className="rounded-2xl bg-green-50 px-5 py-3 text-center text-sm font-black text-green-700"
                      >
                        Chat WhatsApp Kedai
                      </a>
                    ) : null}
      ...
      {view === "menu" && totalItems > 0 ? (
        <div className="fixed inset-x-0 bottom-4 z-30 px-4">
          <button
            onClick={goToCart}
            className="mx-auto flex w-full max-w-xl items-center justify-between rounded-[1.6rem] bg-saung-dark px-5 py-4 text-white shadow-2xl shadow-red-950/30 transition hover:bg-saung-red"
          >
            <span className="text-left">
              <b className="block text-sm">Lihat Keranjang</b>
              <span className="text-xs text-white/70">
                {totalItems} item siap checkout
              </span>
            </span>
            <span className="rounded-2xl bg-saung-yellow px-4 py-2 text-sm font-black text-saung-dark">
              {rupiah(total)}
            </span>
          </button>
        </div>
  ...
  return (
    <article className="group overflow-hidden rounded-[2.1rem] border border-white/70 bg-white shadow-xl shadow-orange-950/5 transition duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-orange-950/10">
      <div className="relative h-52 overflow-hidden bg-gradient-to-br from-yellow-100 via-orange-100 to-red-100">
        ...
        {product.badge ? (
          <span className="absolute left-4 top-4 rounded-full bg-saung-yellow px-3 py-1 text-[11px] font-black text-saung-dark shadow-lg">
            {product.badge}
          </span>
        ) : null}
      </div>
      <div className="p-5">
        <h3 className="line-clamp-2 min-h-[3.2rem] text-xl font-black leading-tight text-saung-dark">
          {product.name}
        </h3>
        <p className="mt-2 line-clamp-2 min-h-[2.5rem] text-sm font-semibold leading-relaxed text-orange-950/60">
          {product.description || "Menu favorit Kedai Saung Bambu."}
        </p>
        <div className="mt-5 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-2xl font-black text-saung-red">
              {rupiah(product.price)}
            </p>
          </div>
          <button
            onClick={onAdd}
            className="rounded-2xl bg-saung-dark px-5 py-3 text-sm font-black text-white shadow-lg transition hover:bg-saung-red"
          >
            Tambah
          </button>
        </div>
    ...
    <div className="fixed right-4 top-24 z-50 grid w-[calc(100%-2rem)] max-w-sm gap-3 sm:top-6">
      {toasts.map((toast) => {
        ...
        return (
          <div
            key={toast.id}
            className={`rounded-[1.4rem] border p-4 shadow-2xl backdrop-blur ${styles}`}
          >
            <div className="flex items-start gap-3">
              ...
              <button
                ...
                className="rounded-full px-2 font-black opacity-60 hover:opacity-100"
              >
                ×
              </button>
            </div>
...
function CustomerProcessBanner({
  title,
  description,
}: {
  title: string;
  ...
  return (
    <div className="fixed inset-0 z-[90] grid place-items-center bg-saung-dark/55 px-4 backdrop-blur-xl">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,rgba(255,194,61,0.25),transparent_28%),radial-gradient(circle_at_80%_80%,rgba(193,29,45,0.22),transparent_30%)]" />
      <div className="relative w-full max-w-sm overflow-hidden rounded-[2.25rem] border border-white/70 bg-white/95 p-6 text-center text-saung-dark shadow-2xl shadow-red-950/35 backdrop-blur-2xl">
        ...
        <p className="relative mx-auto mt-2 max-w-xs text-sm font-semibold leading-relaxed text-orange-950/60">
          {description}
        </p>
        ...
        <p className="relative mt-3 text-xs font-bold text-orange-950/45">
          Tunggu sebentar ya, jangan tutup halaman ini.
        </p>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-40 grid place-items-center bg-saung-dark/55 px-4 backdrop-blur-md">
      <div className="w-full max-w-md overflow-hidden rounded-[2.4rem] bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-5">
            ...
            <div className="rounded-[1.5rem] bg-orange-50 p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-2xl font-black text-saung-dark">
                    #{dialog.order.order_code}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold text-orange-950/60">Total</p>
                  <p className="font-black text-saung-red">
                    {rupiah(dialog.order.total)}
                  </p>
                </div>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm font-semibold text-orange-950/70">
                <span className="rounded-2xl bg-white px-3 py-2">
                  Meja {dialog.order.table_number}
                </span>
                <span className="rounded-2xl bg-white px-3 py-2">
                  {dialog.order.payment_method === "midtrans"
                    ? "Online"
                    : "Kasir"}
                </span>
              </div>
          ...
          {dialog.redirectUrl ? (
            <a
              href={dialog.redirectUrl}
              className="block w-full rounded-2xl bg-saung-red px-5 py-3 text-center font-black text-white shadow-lg transition hover:bg-saung-dark"
            >
          ...
          {dialog.canRetry && onRetry ? (
            <button
              onClick={onRetry}
              disabled={retrying}
              className="w-full rounded-2xl border-2 border-saung-red bg-white px-5 py-3 font-black text-saung-red shadow-sm transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {retrying ? (
                <span className="inline-flex items-center justify-center gap-2">
                  <MiniSpinner /> Membuka pembayaran...
                </span>
              ) : (
                "Bayar Lagi"
              )}
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <button
              onClick={onClose}
              className="rounded-2xl bg-saung-dark px-5 py-3 font-black text-white"
            >
              Tutup
            </button>
            {whatsappUrl ? (
              <a
                href={whatsappUrl}
                target="_blank"
                className="rounded-2xl bg-green-600 px-5 py-3 text-center font-black text-white"
              >
                Chat Kedai
              </a>
            ) : (
              <Link
                href="/"
                className="rounded-2xl bg-orange-50 px-5 py-3 text-center font-black text-saung-red"
              >
                Lihat Menu
              </Link>
            )}
