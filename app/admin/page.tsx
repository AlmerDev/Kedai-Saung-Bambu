...
const navItems: { key: Tab; label: string; icon: string; desc: string }[] = [
  { key: "dashboard", label: "Dashboard", icon: "📊", desc: "Ringkasan usaha" },
  { key: "orders", label: "Pesanan", icon: "🧾", desc: "Order masuk" },
  { key: "products", label: "Menu", icon: "🍗", desc: "Makanan & minuman" },
  { key: "categories", label: "Kategori", icon: "🗂️", desc: "Kelompok menu" },
  { key: "tables", label: "Meja & QR", icon: "🔳", desc: "QR tiap meja" },
  { key: "settings", label: "Setting", icon: "⚙️", desc: "Profil kedai" }
];
        ...
        <aside className="border-b border-white/10 bg-white/10 p-4 text-white backdrop-blur-2xl lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:p-5">
          ...
          <nav className="no-scrollbar mt-4 flex gap-3 overflow-x-auto lg:grid lg:overflow-visible">
            {navItems.map((item) => (
              ...
              >
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white/14 text-lg">{item.icon}</span>
                <span>
          ...
          <div className="mt-5 flex gap-2">
            <Link href="/" className="flex-1 rounded-2xl bg-white/10 px-4 py-3 text-center text-sm font-black text-white transition hover:bg-white/15">Menu</Link>
            <button onClick={logout} className="rounded-2xl bg-red-500/20 px-4 py-3 text-sm font-black text-red-100 transition hover:bg-red-500/30">Logout</button>
          </div>
        </aside>
                ...
                {editor.type === "product" ? (
                  <form onSubmit={saveProduct} className="grid gap-5 p-5 lg:grid-cols-[220px_1fr]">
                    <div className="space-y-3">
                      ...
                      <p className="text-xs leading-5 text-orange-950/55">Foto akan masuk ke Supabase Storage bucket <b>menu-images</b>.</p>
                    </div>
          ...
          {tab === "dashboard" ? (
            <div className="space-y-6">
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <Metric icon="💰" label="Omzet Hari Ini" value={rupiah(stats.omzetToday)} note="Cash + pembayaran lunas" accent="from-emerald-500 to-teal-400" />
                <Metric icon="🧾" label="Order Hari Ini" value={stats.ordersToday} note={`${stats.pendingOrders} order aktif`} accent="from-saung-red to-saung-orange" />
                <Metric icon="🍽️" label="Menu Aktif" value={`${stats.activeProducts}/${stats.products}`} note={`${stats.categories} kategori`} accent="from-orange-500 to-yellow-400" />
                <Metric icon="🔳" label="Meja QR" value={stats.tables} note={`${stats.unpaidOrders} order belum lunas`} accent="from-slate-800 to-slate-500" />
              </section>
                ...
                <div className="space-y-5">
                  <div className="rounded-[2rem] bg-gradient-to-br from-saung-red via-saung-orange to-saung-yellow p-5 text-white shadow-glow">
                    ...
                    <div className="mt-5 grid gap-3">
                      <button onClick={openProductCreate} className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-saung-red">+ Tambah Menu</button>
                      <button onClick={openTableCreate} className="rounded-2xl bg-white/20 px-4 py-3 text-sm font-black text-white backdrop-blur">+ Tambah Meja QR</button>
                      <button onClick={() => setTab("settings")} className="rounded-2xl bg-saung-dark px-4 py-3 text-sm font-black text-white">Atur Foto Toko</button>
                    </div>
                  </div>
              ...
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row">
                    ...
                    <button onClick={openProductCreate} className="btn-primary">+ Tambah Menu</button>
                  </div>
                </div>
              </div>
                  ...
                  <article key={product.id} className="overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-black/10">
                    ...
                    <div className="p-5">
                      ...
                      <div className="mt-5 grid grid-cols-2 gap-2">
                        <button onClick={() => openProductEdit(product)} className="rounded-2xl bg-saung-dark px-4 py-3 text-sm font-black text-white">Edit</button>
                        <button onClick={() => remove(`/api/admin/products/${product.id}`, product.name)} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-700">Hapus</button>
                      </div>
                    </div>
                  </article>
                ))}
            ...
            <section className="space-y-5">
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <button onClick={openCategoryCreate} className="btn-primary">+ Tambah Kategori</button>
                </div>
              </div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {categories.map((cat) => (
            ...
            <section className="space-y-5">
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <button onClick={openTableCreate} className="btn-primary">+ Tambah Meja</button>
                </div>
              </div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {tables.map((table) => {
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                  ...
                  <div className="mt-4 grid gap-3">
                    <label className="block cursor-pointer rounded-2xl border border-dashed border-saung-orange bg-orange-50 p-4 text-sm font-black text-saung-red transition hover:bg-orange-100">
                      Upload Logo dari File
                      <input type="file" accept="image/*" onChange={(e) => uploadSettingImage("logo_url", e.target.files?.[0])} className="hidden" />
                    </label>
                    <label className="block cursor-pointer rounded-2xl border border-dashed border-saung-orange bg-orange-50 p-4 text-sm font-black text-saung-red transition hover:bg-orange-100">
                      Upload Banner/Hero dari File
              ...
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10 sm:p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-[1.5rem] border border-orange-100 bg-orange-50/70 p-4">
                    <p className="mb-3 text-sm font-black text-saung-dark">Logo URL</p>
                  ...
                  <div className="rounded-[1.5rem] border border-orange-100 bg-orange-50/70 p-4">
                    <p className="mb-3 text-sm font-black text-saung-dark">Banner/Hero URL</p>
    ...
    <article className="overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-black/10">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-orange-50 text-2xl">{icon}</div>
        </div>
