...
function categoryIconClass(name?: string | null, stored?: string | null) {
  ...
  if (
    ...
    value.includes("cemilan") ||
    value.includes("snack")
  )
    return "fa-solid fa-cookie-bite";
  if (value.includes("nasi") || value.includes("paket"))
...
function statusBadge(status: string) {
  if (["lunas", "selesai", "siap"].includes(status))
    return "bg-emerald-50 text-emerald-700 ring-emerald-200";
  if (["baru", "diproses", "menunggu"].includes(status))
    return "bg-amber-50 text-amber-700 ring-amber-200";
  if (["dibatalkan", "gagal", "expire", "refund"].includes(status))
...
function prettyChannel(value?: string | null) {
  ...
  if (["BCA", "BNI", "BRI", "MANDIRI", "PERMATA", "CIMB"].includes(upper))
    return upper;
  return value
    .split(/[\s_-]+/)
    .filter(Boolean)
...
function paymentMethodInfo(
  ...
  const map: Record<string, { icon: string; label: string }> = {
    ...
    bank_transfer: {
      ...
      label: channel ? `VA ${channel}` : "Virtual Account",
    },
    echannel: { icon: "fa-solid fa-building-columns", label: "Mandiri Bill" },
    cstore: {
      icon: "fa-solid fa-store",
      label: channel || "Convenience Store",
    },
  ...
  const info = map[type] || {
    ...
    label: prettyChannel(type) || "Midtrans",
  };
  return {
    icon: info.icon,
    label: info.label,
...
export default function AdminPage() {
  ...
  async function checkAuth() {
    const payload = await api<{ authenticated: boolean }>("/api/auth/me").catch(
      () => ({ authenticated: false }),
    );
  ...
  function playNewOrderSound() {
    if (!notificationEnabledRef.current || typeof window === "undefined")
      return;
    try {
  ...
  async function requestNotificationAccess() {
    ...
    if (typeof window === "undefined" || !("Notification" in window)) {
      ...
      notify(
        "Notifikasi aktif di dashboard. Browser ini belum mendukung notifikasi sistem.",
      );
      return;
    }
    ...
    if (permission === "granted")
      notify("Notifikasi pesanan baru aktif. Jangan tutup halaman admin ya.");
    else if (permission === "denied")
      notify("Notifikasi browser ditolak. Toast di dashboard tetap aktif.");
    else
      notify(
        "Notifikasi dashboard aktif. Izinkan notifikasi browser kalau mau popup sistem.",
      );
  }
  ...
  async function loadAll() {
    ...
    try {
      ...
      setSettingsForm({
        ...
        service_fee_percent: String(
          settingRes.settings.service_fee_percent ?? 0,
        ),
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved =
        window.localStorage.getItem("ksb-admin-notification-enabled") ===
        "true";
  ...
  useEffect(() => {
    if (typeof window !== "undefined")
      window.localStorage.setItem(
        "ksb-admin-notification-enabled",
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        const payload = await api<{ orders: Order[] }>(
          "/api/admin/orders?limit=120",
        );
  ...
  const stats = useMemo(() => {
    ...
    const isPaid = (order: Order) =>
      order.payment_status === "lunas" || order.payment_method === "cash";
    ...
    const paymentSummary = {
      ...
      midtrans: orders.filter((order) => order.payment_method === "midtrans")
        .length,
      qris: orders.filter(
        (order) => (order.payment_type || "").toLowerCase() === "qris",
      ).length,
      va: orders.filter(
        (order) => (order.payment_type || "").toLowerCase() === "bank_transfer",
      ).length,
    };
    ...
    const dailyRevenue = Array.from({ length: 7 }, (_, index) => {
      ...
      const dayOrders = orders.filter(
        (order) => new Date(order.created_at).toDateString() === dayKey,
      );
    ...
    const statusSummary = [
      {
        label: "Baru",
        value: orders.filter((order) => order.status === "baru").length,
        icon: "fa-solid fa-bell-concierge",
      },
      {
        label: "Diproses",
        value: orders.filter((order) => order.status === "diproses").length,
        icon: "fa-solid fa-fire-burner",
      },
      {
        label: "Siap",
        value: orders.filter((order) => order.status === "siap").length,
      ...
      {
        label: "Batal",
        value: orders.filter((order) => order.status === "dibatalkan").length,
    ...
    const paymentChart = [
      ...
      {
        label: "Midtrans",
        value: orders.filter((order) => order.payment_method === "midtrans")
          .length,
        icon: "fa-solid fa-credit-card",
      },
      {
      ...
      },
      {
        ...
        value: orders.filter(
          (order) =>
            (order.payment_type || "").toLowerCase() === "bank_transfer",
        ).length,
        icon: "fa-solid fa-building-columns",
      },
    ];
    ...
    return {
      ...
      pendingOrders: orders.filter((order) =>
        ["baru", "diproses"].includes(order.status),
      ).length,
      readyOrders: orders.filter((order) => order.status === "siap").length,
      doneOrders: orders.filter((order) => order.status === "selesai").length,
      canceledOrders: orders.filter((order) => order.status === "dibatalkan")
        .length,
      unpaidOrders: orders.filter((order) =>
        ["belum_bayar", "menunggu"].includes(order.payment_status),
      ).length,
      paidOrders: paidOrders.length,
      cashOrdersToday: todayOrders.filter(
        (order) => order.payment_method === "cash",
      ).length,
      midtransOrdersToday: todayOrders.filter(
        (order) => order.payment_method === "midtrans",
      ).length,
      paidMidtransToday: todayOrders.filter(
        (order) =>
          order.payment_method === "midtrans" &&
          order.payment_status === "lunas",
      ).length,
      waitingMidtransToday: todayOrders.filter(
        (order) =>
          order.payment_method === "midtrans" &&
          order.payment_status === "menunggu",
      ).length,
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) =>
      [product.name, product.description, product.category?.name, product.badge]
        .join(" ")
        .toLowerCase()
        .includes(keyword),
    );
  ...
  const reportStats = useMemo(() => {
    const paid = reportOrders.filter(
      (order) =>
        order.payment_status === "lunas" || order.payment_method === "cash",
    );
    const items = reportOrders.flatMap((order) =>
    ...
    return {
      ...
      unpaidOrders: reportOrders.filter((order) =>
        ["belum_bayar", "menunggu"].includes(order.payment_status),
      ).length,
      canceledOrders: reportOrders.filter(
  ...
  function reportRowsHtml() {
    const rows = reportOrders
      ...
      .join("");
    return (
      rows || `<tr><td colspan="10">Tidak ada data pada periode ini.</td></tr>`
    );
  }
  ...
  function exportReportExcel() {
    ...
    downloadBlob(
      `rekap-kedai-saung-bambu-${reportFrom}-${reportTo}.xls`,
      `\ufeff${html}`,
  ...
  function openCategoryEdit(category: Category) {
    setCategoryForm({
      id: category.id,
      name: category.name,
      emoji: category.emoji?.startsWith("fa-")
        ? category.emoji
        : categoryIconClass(category.name, category.emoji),
  ...
  async function saveCategory(e: FormEvent) {
    ...
    try {
      ...
      if (categoryForm.id)
        await api(`/api/admin/categories/${categoryForm.id}`, {
          method: "PATCH",
          body,
        });
  ...
  async function saveTable(e: FormEvent) {
    ...
    setMessage("");
    setActionLoading(
      tableForm.id ? "Menyimpan perubahan meja..." : "Menambahkan meja baru...",
    );
    try {
      const body = JSON.stringify(tableForm);
      if (tableForm.id)
        await api(`/api/admin/tables/${tableForm.id}`, {
          method: "PATCH",
          body,
        });
    ...
    try {
      ...
      if (url)
        setSettingsForm((current) =>
          current ? { ...current, [field]: url } : current,
        );
    } catch (err) {
      setMessage(err instanceof Error ? err.message : "Upload gambar gagal.");
    }
  ...
  async function deletePhotoFromStorage(url?: string | null) {
    if (!url) return;
    await api<{ deleted?: boolean; skipped?: boolean; message?: string }>(
  ...
  if (authenticated === null) {
    return (
      <main className="grid min-h-screen place-items-center bg-[#f6f1e8] px-4">
        <div className="rounded-2xl border border-[#eadbc6] bg-white px-6 py-5 font-black text-saung-dark shadow-sm">
          Memuat admin...
        </div>
      </main>
    );
    ...
    return (
      <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#f6f1e8] px-4 py-10">
        ...
        <form
          ...
          className="relative w-full max-w-md rounded-[1.5rem] border border-[#eadbc6] bg-white p-8 text-saung-dark shadow-sm"
        >
          <div className="mb-7 flex items-center gap-4">
            <div className="grid h-16 w-16 place-items-center rounded-3xl bg-saung-yellow text-2xl shadow-glow">
              <i className="fa-solid fa-store" />
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-saung-orange">
                Admin Panel
              </p>
              <h1 className="text-2xl font-black">KEDAI SAUNG BAMBU</h1>
            </div>
          </div>
          <p className="mb-5 text-sm leading-6 text-orange-950/60">
            Masuk untuk mengatur dashboard, pesanan, menu, QR meja, foto toko,
            dan pembayaran.
          </p>
          <input
            value={password}
            ...
            type="password"
            className="w-full rounded-xl border border-[#eadbc6] bg-[#fff8ec] px-4 py-4 text-sm font-bold text-saung-dark outline-none placeholder:text-orange-950/40 focus:ring-4 focus:ring-orange-100"
          ...
          <button
            ...
            className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-saung-red px-5 py-4 font-black text-white shadow-sm transition hover:bg-saung-dark active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70"
          >
            {actionLoading ? (
              <AdminSpinner light />
            ) : (
          ...
          <Link
            href="/"
            className="mt-5 block text-center text-sm font-bold text-saung-red"
          >
    ...
    <main className="admin-gojek-shell human-admin min-h-screen bg-[#f6f1e8] text-saung-dark">
      ...
      <div className="relative grid min-h-screen lg:grid-cols-[300px_1fr]">
        <aside className="admin-gojek-sidebar border-b border-orange-100 bg-white p-4 text-saung-dark lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:p-5">
          <div className="flex items-center gap-4 rounded-[2rem] border border-orange-100 bg-[#fff8ec] p-4 shadow-sm">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-black uppercase tracking-[0.28em] text-saung-orange">
                Admin Suite
              </p>
              <h1 className="truncate text-lg font-black">
                {settings?.store_name || "Saung Bambu"}
              </h1>
            </div>
          </div>
          ...
          <nav className="no-scrollbar mt-4 flex gap-3 overflow-x-auto lg:grid lg:overflow-visible">
            {navItems.map((item) => (
              ...
              >
                <span className="admin-nav-icon grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-lg text-saung-red shadow-sm">
                  <i className={item.icon} />
                </span>
                <span>
                  <span className="block text-sm font-black">{item.label}</span>
                  <span className="admin-nav-desc block text-xs text-orange-950/50">
                    {item.desc}
                  </span>
                </span>
                {item.key === "orders" && unreadOrders > 0 ? (
                  <span className="ml-auto grid h-6 min-w-6 place-items-center rounded-full bg-red-600 px-1 text-xs font-black text-white ring-2 ring-white/30">
                    {unreadOrders}
                  </span>
                ) : null}
              </button>
          ...
          <div className="mt-5 hidden rounded-[2rem] border border-orange-100 bg-[#fff8ec] p-4 lg:block">
            ...
            <div className="mt-3 space-y-2 text-sm text-orange-950/60">
              <p className="flex justify-between">
                ...
                <b className="text-saung-dark">{stats.activeProducts}</b>
              </p>
              <p className="flex justify-between">
                <span>Order aktif</span>
                <b className="text-saung-yellow">{stats.pendingOrders}</b>
              </p>
              <p className="flex justify-between">
                <span>Meja</span>
                <b className="text-saung-dark">{stats.tables}</b>
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 flex gap-2">
            <Link
              href="/"
              className="flex-1 rounded-2xl bg-[#fff4db] px-4 py-3 text-center text-sm font-black text-saung-red transition hover:bg-yellow-50"
            >
        ...
        <section className="admin-content admin-gojek-content min-w-0 p-4 sm:p-6 lg:p-8">
            ...
            <div className="relative p-6 sm:p-7">
              ...
              <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.28em] text-saung-orange">
                      {navItems.find((item) => item.key === tab)?.label ||
                        "Dashboard"}
                    </p>
                    <h2 className="mt-2 text-3xl font-black text-saung-dark sm:text-4xl">
                      {settings?.store_name || "KEDAI SAUNG BAMBU"}
                    </h2>
                    <p className="mt-2 max-w-2xl text-sm leading-6 text-orange-950/65">
                      Kelola operasional kedai dari satu dashboard: pesanan
                      realtime, menu, QR meja, profil toko, dan setting
                      pembayaran.
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    className={`relative flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-black shadow-sm transition hover:scale-[1.02] ${notificationEnabled ? "bg-emerald-600 text-white" : "bg-yellow-100 text-orange-900"}`}
                  >
                    <i
                      className={
                        notificationEnabled
                    ...
                    {unreadOrders ? (
                      <span className="absolute -right-2 -top-2 grid h-6 min-w-6 place-items-center rounded-full bg-red-700 px-1 text-xs text-white ring-2 ring-white">
                        {unreadOrders}
                      </span>
                    ) : null}
                  </button>
                  <button
                    ...
                    className="flex items-center gap-2 rounded-2xl border border-orange-200 bg-white px-4 py-3 text-sm font-black text-saung-dark shadow-sm transition hover:bg-orange-50 disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {loading ? (
                      <AdminSpinner />
                    ) : (
                  ...
                  <Link
                    href="/"
                    className="rounded-2xl bg-saung-dark px-4 py-3 text-sm font-black text-white shadow-sm transition hover:scale-[1.02]"
                  >
          ...
          {loading ? (
            <div className="mb-5 flex items-center gap-3 rounded-3xl border border-yellow-200 bg-yellow-50 p-4 text-sm font-black text-orange-700 shadow">
              <AdminSpinner />
              Memuat data terbaru...
            </div>
          ) : null}
          ...
          {deleteTarget ? (
            <div className="fixed inset-0 z-50 grid place-items-center bg-saung-dark/60 px-4 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-[2rem] bg-white p-6 shadow-2xl">
                <div className="mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-red-50 text-2xl text-red-700">
                ...
                <p className="mt-2 text-sm leading-6 text-orange-950/70">
                  Data <b>{deleteTarget.label}</b> akan dihapus dari database.
                  Aksi ini tidak bisa dibatalkan.
                </p>
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  <button
                    onClick={() => setDeleteTarget(null)}
                    className="rounded-2xl bg-orange-50 px-5 py-3 font-black text-saung-red"
                  >
                    Batal
                  </button>
                  <button
                    ...
                    disabled={!!actionLoading}
                    className="flex items-center justify-center gap-2 rounded-2xl bg-red-700 px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {actionLoading ? <AdminSpinner light /> : null}Ya, Hapus
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="fixed inset-0 z-50 grid place-items-center bg-saung-dark/60 px-4 py-8 backdrop-blur-sm">
              <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[2.2rem] bg-white shadow-2xl">
                <div className="sticky top-0 z-10 flex items-center justify-between border-b border-orange-100 bg-white/95 p-5 backdrop-blur">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.28em] text-saung-orange">
                      {editor.mode === "create" ? "Form Tambah" : "Form Edit"}
                    </p>
                    <h2 className="text-2xl font-black text-saung-dark">
                      {editor.type === "product"
                        ? editor.mode === "create"
                  ...
                  <button
                    onClick={closeEditor}
                    className="grid h-11 w-11 place-items-center rounded-2xl bg-orange-50 text-xl font-black text-saung-red"
                  >
                    ×
                  </button>
                </div>
                    ...
                    <div className="space-y-2">
                      <div className="overflow-hidden rounded-[1.6rem] border border-orange-100 bg-orange-50">
                        {productForm.image_url ? (
                          ...
                          <div className="grid h-56 place-items-center text-4xl text-saung-red">
                      ...
                      <div className="grid gap-2">
                        <label className="block cursor-pointer rounded-2xl border border-dashed border-saung-orange bg-orange-50 p-4 text-center text-sm font-black text-saung-red transition hover:bg-orange-100">
                          {actionLoading.includes("Mengupload foto menu") ? (
                            <AdminSpinner />
                          ) : (
                        ...
                        {productForm.image_url ? (
                          ...
                          >
                            ...
                            {deletingPhoto ? "Menghapus..." : "Hapus foto menu"}
                          </button>
                        ) : null}
                      </div>
                      <p className="text-xs leading-5 text-orange-950/55">
                        Foto akan masuk ke Supabase Storage bucket{" "}
                        <b>menu-images</b>. Tombol hapus juga menghapus file
                        dari Storage kalau foto berasal dari upload.
                      </p>
                    </div>
                    <div className="space-y-4">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <button
                          type="button"
                          onClick={closeEditor}
                          className="rounded-2xl bg-orange-50 px-4 py-3 font-black text-saung-red"
                        >
                          Batal
                        </button>
                        <button
                        ...
                        >
                          {actionLoading ? <AdminSpinner light /> : null}
                          {editor.mode === "create"
                            ? "Tambah Menu"
                            : "Simpan Perubahan"}
                        </button>
            ...
            <div className="dashboard-no-blank space-y-4">
              <section className="grid gap-4 2xl:grid-cols-[0.95fr_1.05fr]">
                <article className="rounded-[1.35rem] border border-orange-100 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-black uppercase tracking-[0.22em] text-saung-orange">
                        Ringkasan hari ini
                      </p>
                      <h2 className="mt-2 text-3xl font-black text-saung-dark">
                        {rupiah(stats.omzetToday)}
                      </h2>
                      <p className="mt-1 text-xs font-semibold text-orange-950/55">
                        {stats.ordersToday} order hari ini •{" "}
                        {stats.pendingOrders} aktif
                      </p>
                    </div>
                    <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-red-50 text-saung-red">
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-3">
                    <MiniMetric
                      label="Bulan ini"
                      value={rupiah(stats.omzetMonth)}
                    />
                    <MiniMetric label="Total" value={rupiah(stats.omzetAll)} />
                    <MiniMetric
                      label="Rata-rata"
                      value={rupiah(stats.avgTicket)}
                    />
                  </div>
                </article>
                ...
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  ...
                  <CompactMetric
                    ...
                    note={`${stats.categories} kategori`}
                  />
                  <CompactMetric
                    icon="fa-solid fa-qrcode"
                    label="Meja QR"
              ...
              <section className="rounded-[1.35rem] border border-orange-100 bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-center">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h3 className="mt-1 truncate text-2xl font-black text-saung-dark">
                      {settings?.store_name || "KEDAI SAUNG BAMBU"}
                    </h3>
                    <p className="mt-1 line-clamp-2 text-sm leading-6 text-orange-950/60">
                      {settings?.tagline ||
                        "Logo profil akan tampil di halaman customer."}
                    </p>
                  </div>
                  <button
                    onClick={() => selectTab("settings")}
                    className="shrink-0 rounded-2xl bg-saung-dark px-4 py-3 text-sm font-black text-white"
                  >
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl bg-orange-50 p-4">
                    ...
                    <p className="mt-1 line-clamp-2 text-sm font-bold text-orange-950/70">
                      {settings?.address || "Belum diisi"}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-yellow-50 p-4">
                    ...
                    <p className="mt-1 text-sm font-bold text-orange-950/70">
                      {settings?.opening_hours || "Belum diisi"}
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="rounded-[1.35rem] border border-orange-100 bg-white p-4 shadow-sm">
                <div className="mb-4 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h3 className="text-xl font-black text-saung-dark">
                      Pesanan terbaru
                    </h3>
                  </div>
                  <button
                    onClick={() => selectTab("orders")}
                    className="rounded-2xl bg-orange-50 px-4 py-2 text-sm font-black text-saung-red"
                  >
                    Lihat semua
                  </button>
                </div>
                <div className="overflow-hidden rounded-[1.25rem] border border-orange-100">
                  {recentOrders.length ? (
                    recentOrders.slice(0, 4).map((order) => (
                      <div
                        key={order.id}
                        className="grid gap-3 border-b border-orange-100 bg-white p-3 last:border-b-0 md:grid-cols-[1fr_auto_auto] md:items-center"
                      >
                        <div>
                          <p className="text-xs font-black uppercase tracking-wider text-orange-700">
                            #{order.order_code}
                          </p>
                          <h4 className="font-black text-saung-dark">
                            Meja {order.table_number || "-"} •{" "}
                            {order.customer_name}
                          </h4>
                          <p className="text-xs text-orange-950/55">
                        ...
                        <span
                          className={`w-fit rounded-full px-3 py-1 text-xs font-black ring-1 ${statusBadge(order.status)}`}
                        >
                          {order.status}
                        </span>
                    ...
                    <div className="p-6 text-sm font-bold text-orange-950/60">
              ...
              <section className="grid gap-4 xl:grid-cols-2">
                <DashboardBarChart
                  ...
                  items={stats.statusSummary}
                />
                <DashboardBarChart
                  title="Metode Bayar"
                  description="Kasir, Midtrans, QRIS, dan VA."
                  items={stats.paymentChart}
                />
              </section>
              ...
              <section className="grid gap-4 xl:grid-cols-2">
                  ...
                  <div className="rounded-[1.35rem] border border-emerald-100 bg-emerald-50 p-4 shadow-sm">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <h3 className="mt-1 text-lg font-black text-saung-dark">
                          {notificationEnabled ? "Aktif" : "Belum aktif"}
                        </h3>
                        <p className="mt-1 text-xs leading-5 text-orange-950/60">
                          Aktifkan supaya admin mendapat bunyi dan popup saat
                          order masuk.
                        </p>
                        <button
                          onClick={requestNotificationAccess}
                          className={`mt-3 rounded-2xl px-4 py-2.5 text-sm font-black ${notificationEnabled ? "bg-emerald-600 text-white" : "bg-saung-dark text-white"}`}
                        >
                  ...
                  <div className="rounded-[1.35rem] bg-saung-red p-4 text-white shadow-sm">
                    ...
                    <div className="mt-4 grid gap-2 sm:grid-cols-3 xl:grid-cols-1">
                      <button
                        ...
                        className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-saung-red"
                      >
                      ...
                      <button
                        ...
                        className="rounded-2xl bg-white/15 px-4 py-3 text-sm font-black text-saung-red"
                      >
                      ...
                      <button
                        ...
                        className="rounded-2xl bg-saung-dark px-4 py-3 text-sm font-black text-white"
                      >
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-[1.35rem] border border-orange-100 bg-white p-4 shadow-sm">
                    ...
                    <h3 className="text-lg font-black text-saung-dark">
                      Menu terbaru
                    </h3>
                    <div className="mt-3 space-y-2">
                      {latestProducts.slice(0, 4).map((product) => (
                        <div
                          key={product.id}
                          className="flex items-center gap-3 rounded-2xl bg-orange-50 p-3"
                        >
                          <div className="grid h-11 w-11 place-items-center overflow-hidden rounded-xl bg-white text-lg">
                            {product.image_url ? (
                              <img
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-sm font-black text-saung-dark">
                              {product.name}
                            </p>
                            <p className="text-xs text-orange-950/55">
          ...
          {tab === "orders" ? (
            <section className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
              <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.25em] text-saung-orange">
                    Order Management
                  </p>
                ...
                <button
                  onClick={loadAll}
                  disabled={loading || !!actionLoading}
                  className="flex items-center justify-center gap-2 rounded-2xl bg-orange-50 px-4 py-3 text-sm font-black text-saung-red disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {loading ? (
                    <AdminSpinner />
                  ) : (
              ...
              <div className="grid gap-4">
                {orders.map((order) => (
                  <article
                    key={order.id}
                    className="rounded-[1.6rem] border border-orange-100 bg-white p-4"
                  >
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${statusBadge(order.status)}`}
                          >
                            {order.status}
                          </span>
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${statusBadge(order.payment_status)}`}
                          >
                        ...
                        <p className="mt-1 text-sm text-orange-950/60">
                          {new Date(order.created_at).toLocaleString("id-ID")} •{" "}
                          {order.customer_phone || "Tanpa nomor"}
                        </p>
                        {order.note ? (
                          <p className="mt-2 rounded-2xl bg-white p-3 text-sm text-orange-950/75">
                            Catatan: {order.note}
                          </p>
                        ) : null}
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-3">
                      <PaymentInfoCard
                        ...
                        value={paymentMethodInfo(order).label}
                        note={paymentMethodInfo(order).detail}
                      />
                      <PaymentInfoCard
                        icon="fa-solid fa-circle-check"
                      ...
                      <PaymentInfoCard
                        ...
                        note={order.customer_phone || "Tanpa nomor HP"}
                      />
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-orange-100 pt-4">
                      ...
                      <button
                        ...
                        className="rounded-2xl bg-red-50 px-4 py-2 text-sm font-bold text-red-700"
                      >
                        Hapus
                      </button>
                    </div>
            ...
            <section className="space-y-5">
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-orange-950/60">
                      Form tambah dan edit sekarang dipisah ke modal khusus,
                      jadi list menu tetap rapi.
                    </p>
                  </div>
                  <div className="flex flex-col gap-3 sm:flex-row">
                    ...
                    />
                    <button onClick={openProductCreate} className="btn-primary">
                      <i className="fa-solid fa-plus mr-2" />
                      Tambah Menu
                    </button>
                  </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {filteredProducts.map((product) => (
                  <article
                    key={product.id}
                    className="overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-black/10"
                  >
                    <div className="relative h-44 bg-orange-50">
                      {product.image_url ? (
                        ...
                        <div className="grid h-full place-items-center text-4xl text-saung-red">
                          <i className="fa-solid fa-utensils" />
                        </div>
                      )}
                      <div className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-black text-saung-red shadow">
                    ...
                    <div className="p-5">
                      ...
                      <p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-orange-950/60">
              ...
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-orange-950/60">
                      Tambah dan edit kategori tampil di form terpisah.
                    </p>
                  </div>
                  <button onClick={openCategoryCreate} className="btn-primary">
                    <i className="fa-solid fa-plus mr-2" />
                    Tambah Kategori
                  </button>
                </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {categories.map((cat) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-orange-50 text-2xl text-saung-red">
                          <i
                            className={categoryIconClass(cat.name, cat.emoji)}
                          />
                        </div>
                      ...
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-black ${cat.is_active ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                      >
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-2">
                      ...
                      <button
                        onClick={() =>
                          remove(`/api/admin/categories/${cat.id}`, cat.name)
                        }
                        className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-700"
                      >
                        Hapus
              ...
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-orange-950/60">
                      Tambah dan edit meja dibuat terpisah supaya QR list tetap
                      bersih.
                    </p>
                  </div>
                  <button onClick={openTableCreate} className="btn-primary">
                    <i className="fa-solid fa-plus mr-2" />
                    Tambah Meja
                  </button>
                </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {tables.map((table) => {
                  ...
                  return (
                    <article
                      key={table.id}
                      className="rounded-[2rem] bg-white p-5 text-center shadow-2xl shadow-black/10"
                    >
                      ...
                      <h3 className="mt-4 text-xl font-black">
                        {table.label || `Meja ${table.table_number}`}
                      </h3>
                      <p className="mt-1 break-all text-xs text-orange-950/55">
                        {url}
                      </p>
                      <div className="mt-5 grid grid-cols-2 gap-2">
                        <button
            ...
            <section className="space-y-5">
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-orange-950/60">
                      Filter periode, cek omzet, dan export laporan ke PDF atau
                      Excel.
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[160px_160px_auto_auto]">
                    ...
                    <button
                      ...
                      className="rounded-2xl bg-saung-red px-5 py-3 text-sm font-black text-white"
                    >
              ...
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <Metric
                  ...
                  label="Omzet Periode"
                  value={rupiah(reportStats.revenue)}
                  note="Order lunas + bayar kasir"
                  accent="from-emerald-500 to-teal-400"
                />
                <Metric
                  icon="fa-solid fa-receipt"
                  label="Total Order"
                  value={reportStats.totalOrders}
                  note={`${reportStats.paidOrders} order terbayar`}
                  accent="from-saung-red to-saung-orange"
                />
                <Metric
                  icon="fa-solid fa-hourglass-half"
                  label="Belum Lunas"
                  value={reportStats.unpaidOrders}
                ...
                <Metric
                  icon="fa-solid fa-ban"
                  label="Dibatalkan"
                  value={reportStats.canceledOrders}
                  note="Order batal periode ini"
                ...
                <div className="overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-black/10">
                  <div className="border-b border-orange-100 p-5">
                    <h4 className="text-xl font-black">
                      <i className="fa-solid fa-table mr-2 text-saung-red" />
                      Detail Order
                    </h4>
                    <p className="mt-1 text-xs font-semibold text-orange-950/55">
                      Data ini yang ikut ke file PDF/Excel.
                    </p>
                  </div>
                      ...
                      <tbody>
                        {reportOrders.length ? (
                          reportOrders.map((order) => (
                            <tr
                              key={order.id}
                              className="border-b border-orange-100 odd:bg-orange-50/70"
                            >
                              <td className="p-3 font-semibold text-orange-950/65">
                              ...
                              <td className="p-3 text-right font-black text-saung-red">
                                {rupiah(order.total)}
                              </td>
                            </tr>
                          ))
                          ...
                          <tr>
                            <td
                              colSpan={7}
                              className="p-6 text-center font-bold text-orange-950/55"
                            >
                ...
                <div className="space-y-5">
                  <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                    <div className="mb-4 flex items-center justify-between">
                      <h4 className="text-xl font-black">
                        <i className="fa-solid fa-trophy mr-2 text-saung-orange" />
                        Produk Terjual
                      </h4>
                      <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-saung-red">
                        {reportStats.products.length} menu
                      </span>
                    </div>
                    <div className="space-y-2">
                      {reportStats.products.length ? (
                        reportStats.products.slice(0, 10).map((item, index) => (
                          <div
                            key={item.name}
                            className="flex items-center justify-between rounded-2xl bg-orange-50 px-4 py-3 text-sm"
                          >
                            <span className="font-black">
                              #{index + 1} {item.name}
                            </span>
                            <span className="text-right">
                              <b className="text-saung-red">{item.qty}x</b>
                              <small className="ml-2 font-bold text-orange-950/55">
                                {rupiah(item.revenue)}
                              </small>
                            </span>
                          </div>
                        ...
                        <p className="rounded-2xl bg-orange-50 p-4 text-sm font-bold text-orange-950/60">
                  ...
                  <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                    <h4 className="text-xl font-black">
                      <i className="fa-solid fa-circle-info mr-2 text-saung-red" />
                      Catatan Export
                    </h4>
                    <p className="mt-2 text-sm leading-6 text-orange-950/65">
                      Tombol PDF akan membuka jendela print browser. Pilih{" "}
                      <b>Save as PDF</b>. Tombol Excel mengunduh file{" "}
                      <b>.xls</b> yang bisa dibuka lewat Microsoft Excel atau
                      Google Sheets.
                    </p>
                  </div>
                </div>
            ...
            >
              <div className="space-y-5">
                <div className="rounded-[2rem] bg-white p-6 text-center shadow-2xl shadow-black/10">
                  ...
                  <p className="mt-5 text-xs font-black uppercase tracking-[0.25em] text-saung-orange">
                    Preview Profil Customer
                  </p>
                  <h3 className="mt-2 text-2xl font-black text-saung-dark">
                    {settingsForm.store_name || "KEDAI SAUNG BAMBU"}
                  </h3>
                  <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-orange-950/65">
                    {settingsForm.tagline ||
                      "Logo profil yang kamu upload akan tampil di halaman customer dan admin."}
                  </p>
                </div>

                <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10">
                  <h3 className="text-xl font-black">
                    Upload Logo / Profile Toko
                  </h3>
                  <p className="mt-1 text-sm leading-6 text-orange-950/60">
                    Fitur banner dihapus. Sekarang tampilan customer memakai
                    logo/profile toko saja.
                  </p>
                  <div className="mt-4 rounded-[1.5rem] border border-orange-100 bg-orange-50/60 p-3">
                    <label className="block cursor-pointer rounded-2xl border border-dashed border-saung-orange bg-white p-4 text-sm font-black text-saung-red transition hover:bg-orange-50">
                      {actionLoading.includes("foto toko") ? (
                        <AdminSpinner />
                      ) : (
                        <i className="fa-solid fa-upload mr-2" />
                      )}
                      Upload Logo/Profile dari File
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) =>
                          uploadSettingImage("logo_url", e.target.files?.[0])
                        }
                        className="hidden"
                      />
                    </label>
              ...
              <div className="rounded-[2rem] bg-white p-5 shadow-2xl shadow-black/10 sm:p-6">
                <div className="mb-5">
                  ...
                  <p className="mt-1 text-sm text-orange-950/60">
                    Bisa isi URL logo manual atau upload logo lewat file.
                  </p>
                </div>
                <div className="space-y-4">
                  <input
                  ...
                  <input
                    value={settingsForm.service_fee_percent}
                    onChange={(e) =>
                      setSettingsForm({
                        ...settingsForm,
                  ...
                  <div className="rounded-[1.5rem] border border-orange-100 bg-orange-50/70 p-4">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <p className="text-sm font-black text-saung-dark">
                        Logo URL
                      </p>
                      {settingsForm.logo_url ? (
                        <button
                  ...
                  >
                    {actionLoading ? <AdminSpinner light /> : null}Simpan
                    Setting Toko
                  </button>
                  <p className="rounded-2xl bg-orange-50 p-4 text-sm leading-6 text-orange-950/70">
                    Password admin tetap diganti dari file <b>.env.local</b> /
                    Vercel ENV: <b>ADMIN_PASSWORD</b>.
                  </p>
                </div>
              </div>
            </form>
...
function SalesTrendChart({
  ...
  data: Array<{ label: string; date: string; revenue: number; orders: number }>;
}) {
  const width = 720;
  const height = 245;
  const paddingX = 42;
  ...
  const points = data.map((item, index) => {
    ...
    const y =
      paddingTop +
      plotHeight -
      (Number(item.revenue || 0) / maxRevenue) * plotHeight;
    return { ...item, x, y };
  });
  ...
  const bestDay = data.reduce(
    ...
    data[0] || { label: "-", date: "-", revenue: 0, orders: 0 },
  );
  ...
  return (
    <article className="dashboard-chart-card self-start rounded-[1.35rem] border border-orange-100 bg-white p-4 shadow-sm">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-saung-orange">
            Grafik Omzet
          </p>
          <h3 className="mt-1 text-2xl font-black text-saung-dark">
            Tren 7 hari terakhir
          </h3>
          <p className="mt-1 text-xs font-semibold text-orange-950/55">
            Omzet dihitung dari order lunas dan bayar kasir.
          </p>
        </div>
        <div className="rounded-2xl bg-orange-50 px-4 py-3 text-right">
          <p className="text-[11px] font-black uppercase tracking-wider text-orange-950/45">
            Total 7 hari
          </p>
          <p className="text-xl font-black text-saung-red">
            {rupiah(totalRevenue)}
      ...
      <div className="overflow-hidden rounded-[1.4rem] border border-orange-100 bg-[#fffaf2] p-3">
        <svg
          ...
          role="img"
          aria-label="Grafik omzet 7 hari terakhir"
        >
          {[0, 1, 2, 3].map((line) => {
          ...
          {points.map((point) => (
            <g key={`${point.date}-${point.label}`}>
              ...
              <text
                ...
                y={height - 20}
                textAnchor="middle"
                className="fill-[#7a4a35] text-[13px] font-bold"
              >
                {point.label}
              </text>
              <title>
                {point.date}: {rupiah(point.revenue)} • {point.orders} order
              </title>
            </g>
          ))}
        </svg>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-3">
        ...
        <ChartStat
          ...
          value={`${bestDay.label}, ${rupiah(bestDay.revenue)}`}
        />
      </div>
    </article>
  );
    ...
    <article className="dashboard-chart-card self-start rounded-[1.35rem] border border-orange-100 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold leading-5 text-orange-950/55">
            {description}
          </p>
        </div>
        <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-orange-50 text-saung-red">
      ...
      <div className="space-y-2">
        {items.map((item) => {
          ...
          return (
            <div
              key={item.label}
              className="rounded-2xl border border-orange-100 bg-[#fffaf2] p-2.5"
            >
              <div className="mb-2 flex items-center justify-between gap-3 text-sm">
                <span className="min-w-0 font-black text-saung-dark">
                  <i className={`${item.icon} mr-2 text-saung-red`} />
                  {item.label}
                </span>
                <span className="shrink-0 text-right font-black text-saung-red">
                  {item.value}{" "}
                  <small className="font-bold text-orange-950/45">
                    ({percent}%)
                  </small>
                </span>
              </div>
              <div className="h-3 overflow-hidden rounded-full bg-orange-100">
                <div
                  className="h-full rounded-full bg-saung-red"
                  style={{ width: `${width}%` }}
                />
              </div>
            </div>
    ...
    <article className="dashboard-chart-card self-start rounded-[1.35rem] border border-orange-100 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-xl font-black text-saung-dark">
            Menu Terlaris
          </h3>
          <p className="mt-1 text-xs font-semibold text-orange-950/55">
        ...
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-yellow-50 text-yellow-700">
      ...
      <div className="space-y-2">
        {items.length ? (
          items.map((item, index) => {
            const width = Math.max(
              8,
              Math.round((Number(item.qty || 0) / maxQty) * 100),
            );
            return (
              <div
                key={item.name}
              ...
              >
                <div className="mb-2 flex items-start justify-between gap-3 text-sm">
                  <span className="min-w-0 font-black text-saung-dark">
                    <i className="fa-solid fa-ranking-star mr-2 text-saung-orange" />
                    #{index + 1} {item.name}
                  </span>
                  <span className="shrink-0 text-right">
                    <b className="block text-saung-red">{item.qty}x</b>
                    <small className="font-bold text-orange-950/55">
                      {rupiah(item.revenue)}
                    </small>
                  </span>
                </div>
...
function ChartStat({
  ...
  return (
    <div className="rounded-2xl border border-orange-100 bg-white p-3">
      <p className="text-[11px] font-black uppercase tracking-wider text-orange-950/45">
        <i className={`${icon} mr-2 text-saung-red`} />
        {label}
      </p>
      <p className="mt-1 truncate text-sm font-black text-saung-dark">
        {value}
      </p>
    </div>
  );
  ...
  return (
    <div className="fixed right-4 top-4 z-[90] w-[calc(100%-2rem)] max-w-md overflow-hidden rounded-[2rem] border border-yellow-200 bg-white shadow-2xl shadow-black/25 sm:right-6 sm:top-6">
      <div className="relative overflow-hidden bg-saung-red p-5 text-white">
        ...
        <button
          onClick={onClose}
          className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-white/15 text-lg font-black text-white hover:bg-white/25"
        >
          ×
        </button>
      ...
      <div className="grid gap-3 bg-white p-5 sm:grid-cols-[1fr_auto] sm:items-center">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-orange-950/45">
            Total pesanan
          </p>
          <p className="mt-1 text-xl font-black text-saung-red">
            {rupiah(order.total)}
          </p>
          <p className="mt-1 text-xs font-bold text-orange-950/45">
            Masuk: {new Date(order.created_at).toLocaleString("id-ID")}
          </p>
        </div>
        <button
          onClick={onOpen}
          className="rounded-2xl bg-saung-dark px-5 py-3 text-sm font-black text-white shadow-sm transition hover:scale-[1.02]"
        >
...
function AdminSpinner({
  ...
  return (
    <span
      className={`inline-block ${size} animate-spin rounded-full border-2 ${light ? "border-white/35 border-t-white" : "border-saung-red/25 border-t-saung-red"}`}
    />
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[85] grid place-items-center bg-saung-dark/55 px-4 backdrop-blur-xl">
      ...
      <div className="relative w-full max-w-sm overflow-hidden rounded-[2.25rem] border border-white/70 bg-white/95 p-6 text-center text-saung-dark shadow-2xl shadow-black/35 backdrop-blur-2xl">
        ...
        <p className="relative mt-5 text-xs font-black uppercase tracking-[0.28em] text-saung-orange">
          Sedang proses
        </p>
        ...
        <p className="relative mt-3 text-xs font-bold text-orange-950/45">
...
function PaymentChip({ order }: { order: Order }) {
  ...
  return (
    <span
      className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1 text-xs font-black ring-1 ${info.tone}`}
    >
...
function PaymentInfoCard({
  ...
  return (
    <div className="rounded-[1.35rem] border border-orange-100 bg-white px-4 py-3 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-orange-50 text-saung-red">
          <i className={icon} />
        </div>
        <div className="min-w-0">
          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-orange-950/45">
            {label}
          </p>
          <h4 className="mt-1 truncate text-sm font-black text-saung-dark">
            {value}
          </h4>
          {note ? (
            <p className="mt-1 truncate text-xs text-orange-950/50">{note}</p>
          ) : null}
        </div>
      </div>
    </div>
...
function MiniMetric({
  ...
  return (
    <div className="rounded-2xl border border-orange-100 bg-[#fffaf2] px-3 py-2">
      <p className="text-[10px] font-black uppercase tracking-wider text-orange-950/45">
        {label}
      </p>
      <p className="mt-0.5 truncate text-sm font-black text-saung-dark">
        {value}
      </p>
    </div>
  );
  ...
  return (
    <article className="rounded-[1.15rem] border border-orange-100 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-xs font-black text-orange-950/55">
            {label}
          </p>
          <div className="mt-1 flex items-end gap-2">
            <h3 className="truncate text-2xl font-black leading-none text-saung-dark">
              {value}
            </h3>
            <span className="mb-0.5 truncate text-[11px] font-bold text-orange-950/45">
              {note}
            </span>
          </div>
        </div>
        <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-orange-50 text-saung-red">
          <i className={icon} />
        </div>
      </div>
    </article>
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black text-saung-dark sm:text-3xl">
              {value}
            </h2>
          </div>
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-orange-50 text-xl text-saung-red">
            <i className={icon} />
          </div>
        </div>
